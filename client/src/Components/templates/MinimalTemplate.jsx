          ...
          <div className="space-y-4">
            {data.project.map((proj, index) => (
              <div key={index} className="flex flex-col gap-1 break-inside-avoid">
                ...
                {proj.type && <span className="text-xs uppercase tracking-widest text-gray-500">{proj.type}</span>}
                {proj.description && (
            ...
            {data.education.map((edu, index) => (
              <div key={index} className="flex justify-between items-baseline break-inside-avoid">
                <div>
                  ...
                  {edu.cgpa && !edu.has_backlogs && <p className="text-xs text-gray-500 mt-1">CGPA: {edu.cgpa}</p>}
                  {edu.has_backlogs && <p className="text-xs text-gray-500 mt-1">Backlogs</p>}
                  {edu.description && (
                    <p className="text-sm text-gray-700 mt-1 whitespace-pre-line leading-relaxed">{edu.description}</p>
                  )}
                </div>
