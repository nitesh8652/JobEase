              ...
              <div className="space-y-4 text-sm">
                {data.education.map((edu, index) => (
                  <div key={index}>
                    ...
                    {edu.institute && <p className="text-zinc-600">{edu.institute}</p>}
                    <p className="text-xs text-zinc-500">
                      {formatDate(edu.start_date)} –{" "}
                      {edu.is_current ? "Present" : formatDate(edu.end_date)}
                    </p>
                    {edu.cgpa && !edu.has_backlogs && (
                      <p className="text-xs text-zinc-500">CGPA: {edu.cgpa}</p>
                    )}
                    {edu.has_backlogs && <p className="text-xs text-zinc-500">Backlogs</p>}
                  </div>
                ))}
              </div>
            </section>
