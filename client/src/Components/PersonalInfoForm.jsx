...
const PersonalInfoForm = ({ data, onChange, removeBackground, setRemoveBackground }) => {
    ...
    const fields = [
        ...
        {
            key: "linkedin", label: "LinkedIn Profile", type: "url", required: true, icon: Phone
        },
        {
            key: "website", label: "Personal Website", type: "url", required: true, icon: Globe2
        }
            ...
            {fields.map((field) => {
                ...
                return (
                    <div key={field.key} className='space-y-1 mt-5'>
                        ...
                        <input type="text" value={data[field.key] || ""} onChange={(e) => handleChange(field.key, e.target.value)} className='mt-1 w-full px-3 py-2  border border-gray-400 rounded-lg focus:ring focus:rig-blue-500 focus:border-blue-500 outline-none transition-colorstext-sm ' placeholder={`Enter your  ${field.label.toLocaleLowerCase()}`} required={field.required} />
                    </div>
                )
            })}
